<script lang="ts">
  ...
  function getTransform(mask: Mask, index: number) {
...
<div
  bind:this={container}
  class="absolute w-full h-full top-0 left-0 opacity-30 pointer-events-none"
>
  <svg
  ...
  >
    {#each masks as mask, index (index)}
      <polygon
        ...
        style:transform={getTransform(mask, index)}
